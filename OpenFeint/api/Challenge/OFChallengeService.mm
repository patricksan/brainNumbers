...
{	
	...
	OFRetainedPtr<NSData> retainedData = [OFCompressableData serializedDataFromData:challengeData];
							   ...
							   onFailure:(OFDelegate const&)onFailure
{
	...
	if(challengeDataUrl != nil && ![challengeDataUrl isEqualToString:@""])
	{
		...
		OFHttpRequest* httpRequest = httpService->startRequest(challengeDataUrl, HttpMethodGet, nil, new OFBlobDownloadObserver(onSuccess, onFailure));
		handle = [OFRequestHandle requestHandle:httpRequest];
	}
	return handle;
}
